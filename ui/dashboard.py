...
class MiniDashboard:
    ...
    def __init__(self, controller):
        ...
        # === Risk panel (Auto-Exits + BE + Trailing) ===
        risk = tk.Frame(self.root, bg=COLORS["PANEL"])
        risk.pack(fill="x", padx=10, pady=(0,10))

        # Auto-Exits master toggle (controls BE/Trailing daemon)
        auto_box = tk.Frame(risk, bg=COLORS["PANEL"]); auto_box.pack(fill="x", padx=(10,10), pady=(6,2))
        self.auto_exits = tk.BooleanVar(value=True)
        try:
            if hasattr(self.controller, "get_auto_exits"):
                self.auto_exits.set(bool(self.controller.get_auto_exits()))
        except Exception:
            pass
        tk.Checkbutton(auto_box, text="Auto-Exits (BE/Trailing)", variable=self.auto_exits,
                       command=self.on_auto_exits_toggle, bg=COLORS["PANEL"],
                       fg=COLORS["TEXT"], selectcolor=COLORS["BG"],
                       font=("Segoe UI", 10, "bold")).pack(anchor="w")
    ...
    def on_auto_exits_toggle(self):
        try:
            en = bool(self.auto_exits.get())
            if hasattr(self.controller, "set_auto_exits"):
                self.controller.set_auto_exits(en)
            else:
                setattr(self.controller, "EXIT_DAEMON_DISABLE", (not en))
        except Exception:
            pass
