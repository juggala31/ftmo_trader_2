...
# === AI profile wiring ===
try:
    from ai.ai_config import load_profile, apply_env_from_profile
    _ai_profile = load_profile()
    if _ai_profile:
        apply_env_from_profile(_ai_profile)
        print("[ai][cfg] applied ai_profile.json into env", flush=True)
except Exception as _e:
    print(f"[ai][cfg] WARNING: could not apply ai_profile.json: {_e}", flush=True)
# === /AI profile wiring ===
# [PATCH HEADER v3] phase+force+cooldown+deepwatch
# 1) Hard-disable AI force-once early
os.environ['AI_FORCE_ONCE'] = ''
os.environ['AI_FORCE_SIDE'] = ''
os.environ['AI_FORCE_VOL']  = ''

# 2) Read desired cooldown from env
def _phase_get_cd():
    try:
        v = os.getenv('COOLDOWN_S') or os.getenv('ORDER_COOLDOWN_S') or os.getenv('COOLDOWN') \
            or os.getenv('HB_COOLDOWN_S') or os.getenv('HEARTBEAT_COOLDOWN_S')
        return int(float(v)) if v else None
    except Exception:
        return None

# 3) Shallow apply to globals & top-level controller attrs
def _phase_apply_cd_shallow(cd):
    if cd is None:
        return
    try:
        for _name in ('COOLDOWN_S','cooldown','COOLDOWN','HB_COOLDOWN_S','HEARTBEAT_COOLDOWN_S'):
            try:
                if _name in globals():
                    globals()[_name] = cd
            except Exception:
                pass
        _ctrl = globals().get('controller')
        if _ctrl is not None:
            for _attr in ('cooldown_s','cooldown','COOLDOWN_S','hb_cooldown_s','heartbeat_cooldown_s'):
                if hasattr(_ctrl, _attr):
                    try: setattr(_ctrl, _attr, cd)
                    except Exception: pass
    except Exception:
        pass

# 4) Deep apply: walk attributes of controller (1â€“2 levels) and set any '*cooldown*' ints
def _phase_apply_cd_deep(cd, obj, depth=0, max_depth=2, seen=None):
    if cd is None or obj is None or depth > max_depth:
        return
    ...
    try:
        for name in dir(obj):
            ...
            try:
                ...
                continue
            low = name.lower()
            # Set any attribute that looks like a cooldown value
            if 'cooldown' in low:
                try:
                    setattr(obj, name, cd)
                except Exception:
                    pass
            # Recurse into simple children (objects/modules with a small attr set)
            if depth < max_depth and not isinstance(val, (int, float, str, bytes, tuple, list, dict, set)):
                # avoid modules/functions/methods
                if inspect.ismodule(val) or inspect.isfunction(val) or inspect.ismethod(val):
                    continue
                _phase_apply_cd_deep(cd, val, depth+1, max_depth, seen)
    except Exception:
        pass

# 5) Watchdog: enforce for ~6s to beat late resets from GUI/init
def _phase_cd_watchdog():
    cd = _phase_get_cd()
    if cd is None:
        return
    for _ in range(60):   # ~6s @ 0.1s
        _phase_apply_cd_shallow(cd)
        _ctrl = globals().get('controller')
        if _ctrl is not None:
            _phase_apply_cd_deep(cd, _ctrl, 0, 2, set())
        time.sleep(0.1)

# Apply once immediately, then start watchdog
cd0 = _phase_get_cd()
_phase_apply_cd_shallow(cd0)
try:
    threading.Thread(target=_phase_cd_watchdog, daemon=True).start()
except Exception:
    pass
import os, threading, time
# [PATCH HEADER v2] phase+force+cooldown+watchdog
# 1) Hard-disable AI force-once early
os.environ['AI_FORCE_ONCE'] = ''
os.environ['AI_FORCE_SIDE'] = ''
os.environ['AI_FORCE_VOL']  = ''

# 2) Phase-driven cooldown from env (COOLDOWN_S/ORDER_COOLDOWN_S/COOLDOWN)
def _phase_get_cd():
    try:
        v = os.getenv('COOLDOWN_S') or os.getenv('ORDER_COOLDOWN_S') or os.getenv('COOLDOWN')
        return int(float(v)) if v else None
...
# 3) Apply cooldown to globals/controller immediately
def _phase_apply_cd_once(cd):
    if cd is None:
        return
    try:
        for _name in ('COOLDOWN_S','cooldown','COOLDOWN'):
            try:
                if _name in globals():
                    globals()[_name] = cd
            except Exception:
        ...
        _ctrl = globals().get('controller')
        if _ctrl is not None:
            for _attr in ('cooldown_s','cooldown','COOLDOWN_S'):
                if hasattr(_ctrl, _attr):
                    try:
                        setattr(_ctrl, _attr, cd)
                    except Exception:
                        pass
    except Exception:
        pass

# 4) Watchdog: re-apply for ~5s to beat late resets
def _phase_cd_watchdog():
    cd = _phase_get_cd()
    if cd is None:
        return
    for _ in range(50):  # ~5s @ 0.1s
        _phase_apply_cd_once(cd)
        time.sleep(0.1)

cd0 = _phase_get_cd()
_phase_apply_cd_once(cd0)
try:
    threading.Thread(target=_phase_cd_watchdog, daemon=True).start()
except Exception:
    pass
import os, threading
# [PATCH HEADER] phase+force+cooldown
# Disable AI force-once early (prevents missing 'threading' and unwanted one-shots)
os.environ['AI_FORCE_ONCE'] = ''
os.environ['AI_FORCE_SIDE'] = ''
os.environ['AI_FORCE_VOL']  = ''

# Force cooldown from env (COOLDOWN_S/ORDER_COOLDOWN_S/COOLDOWN)
try:
    _cd_env = os.getenv("COOLDOWN_S") or os.getenv("ORDER_COOLDOWN_S") or os.getenv("COOLDOWN")
    if _cd_env:
        _cd_val = int(float(_cd_env))
        for _name in ("COOLDOWN_S","cooldown","COOLDOWN"):
            try:
                if _name in globals():
                    globals()[_name] = _cd_val
            except Exception:
                pass
        try:
            _ctrl = globals().get("controller")
            if _ctrl is not None:
                for _attr in ("cooldown_s","cooldown","COOLDOWN_S"):
                    if hasattr(_ctrl, _attr):
                        setattr(_ctrl, _attr, _cd_val)
        except Exception:
            pass
except Exception:
    pass
import os
# [PHASE OVERRIDE] strong cooldown env hook
try:
    _cd_env = os.getenv("COOLDOWN_S") or os.getenv("ORDER_COOLDOWN_S") or os.getenv("COOLDOWN")
    if _cd_env:
        _cd_val = int(float(_cd_env))
        # Update common globals used in different builds
        for _name in ("COOLDOWN_S","cooldown","COOLDOWN"):
            try:
                if _name in globals():
                    globals()[_name] = _cd_val
                ...
                pass
        # If the controller object is already bound in globals, update its cooldown attr as well
        try:
            _ctrl = globals().get("controller")
            if _ctrl is not None:
                for _attr in ("cooldown_s","cooldown","COOLDOWN_S"):
    ...
    pass
import os
# [PHASE OVERRIDE] cooldown env hook (added by patch)
try:
    _cd_env = os.getenv("COOLDOWN_S") or os.getenv("ORDER_COOLDOWN_S") or os.getenv("COOLDOWN")
    if _cd_env:
        _cd_val = int(float(_cd_env))
        # Cover common variable names used across builds
        globals()["COOLDOWN_S"] = _cd_val
        globals()["cooldown"]   = _cd_val
        globals()["COOLDOWN"]   = _cd_val
except Exception:
    pass
# ===== [PATCH] [ai/force] env worker =====
try:
    def _ai_force_env_worker():
        # Reads AI_FORCE_* from env; places exactly once via controller.order_market()
        ...
        try:
            ...
            vol  = 0.01
        # Wait up to 5 minutes for controller to exist
        for _ in range(300):
            ctrl = globals().get("controller")
            if ctrl is not None:
                if not getattr(ctrl, "_ai_force_done", False):
                ...
                return
            time.sleep(1.0)
    threading.Thread(target=_ai_force_env_worker, name="ai_force_env", daemon=True).start()
except Exception as e:
    print("[ai/force] init error:", e)
# ===== [/PATCH] [ai/force] env worker =====
import time
...
class TradingController:
    def __init__(self, profile: str, adapter, ftmo_guard, exec_logger: ExecLogger, args: argparse.Namespace):
        ...
        # [AI] export controller and start AI (idempotent)
        try:
            globals()['controller'] = self
            print("[ai] export: controller -> globals()")
            if not getattr(self, "_ai_loop_started", False):
                start_ai_loop(self)
                print("[ai] loop start requested")
        except Exception as _e:
            print("[ai] start_ai_loop failed:", _e)
    ...
    def set_autopause(self, v: bool):
        self._autopause = bool(v)
        print(f"[controller] autopause {self._autopause}")
...
# === [AI HOOK] phase-tracker style alpha loop (DRY_RUN_AI=1) ===
try:
    from ai.alpha_loop import start_ai_loop as _start_ai_loop
    import threading, os
    os.environ.setdefault("DRY_RUN_AI", "1")
    if not globals().get("_AI_LOOP_STARTED"):
        threading.Thread(target=_start_ai_loop, daemon=True).start()
        _AI_LOOP_STARTED = True
        print("[ai] alpha_loop started (background) with DRY_RUN_AI=1")
except Exception as _e:
    print(f"[ai][WARN] failed to start alpha_loop: {_e}")
# === [/AI HOOK] ===
